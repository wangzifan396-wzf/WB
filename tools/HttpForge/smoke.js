...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('HttpForgePure exposed', typeof window.HttpForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('q').value='404'; doc.getElementById('run').click();
  ok('output has 404', (doc.getElementById('out').textContent||'').indexOf('Not Found')>=0);
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('HttpForge smoke: '+pass+' passed, '+fail+' failed');
