...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('TotpForgePure exposed', typeof window.TotpForgePure==='object');
  ok('code element', !!doc.getElementById('code'));
  var code=(doc.getElementById('code').textContent||'');
  ok('code is 6 digits', /^\d{6}$/.test(code));
  ok('bar rendered', !!doc.getElementById('bar'));
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('TotpForge smoke: '+pass+' passed, '+fail+' failed');
