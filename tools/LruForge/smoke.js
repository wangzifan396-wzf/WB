...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('LruForgePure exposed', typeof window.LruForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /keys/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('LruForge smoke: '+pass+' passed, '+fail+' failed');
