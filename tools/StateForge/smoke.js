...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('StateForgePure exposed', typeof window.StateForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('analysis output', /死状态/.test(doc.getElementById('out').textContent));
  ok('run trace', /draft → review/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('StateForge smoke: '+pass+' passed, '+fail+' failed');
