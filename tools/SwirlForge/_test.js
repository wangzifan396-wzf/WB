...
(function(){
  ...
  var ident=A.swirlProcess(img,w,h,{angle:0,radius:50});
  ok('swirl identity', eq(ident,img));
  var sw=A.swirlProcess(img,w,h,{angle:3,radius:50});
  ok('swirl length', sw.length===img.length);
  ok('swirl changed', !eq(sw,img));
  console.log(T.join('\n'));
  console.log('SWIRL_TEST_DONE total='+T.length+' pass='+T.filter(function(x){return x[0]==='P';}).length);
})();
