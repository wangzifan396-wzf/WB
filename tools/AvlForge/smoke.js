...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('AvlForgePure exposed', typeof window.AvlForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('inorder output', /中序: 10, 20, 25, 30, 40, 50/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('AvlForge smoke: '+pass+' passed, '+fail+' failed');
