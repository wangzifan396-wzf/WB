...
setTimeout(() => {
  ...
  ok('GitignoreForgePure exposed', typeof window.GitignoreForgePure === 'object');
  ok('chips rendered', doc.querySelectorAll('#chips .chip').length > 10);
  ok('out present', !!doc.getElementById('out'));
  ok('default Node selected', /node_modules\//.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length === 0);
  if (errs.length) console.error('  js errors:', errs);
  console.log('GitignoreForge smoke: ' + pass + ' passed, ' + fail + ' failed');
