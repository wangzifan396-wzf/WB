...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('PercentileForgePure exposed', typeof window.PercentileForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('p99 output', /p99 = /.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('PercentileForge smoke: '+pass+' passed, '+fail+' failed');
