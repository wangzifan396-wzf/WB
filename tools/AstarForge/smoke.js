...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('AstarForgePure exposed', typeof window.AstarForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('path output', /代价 \d+ 步/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('AstarForge smoke: '+pass+' passed, '+fail+' failed');
