...
(function(){
  ...
  var f=A.filmNoirProcess(img,w,h,{contrast:1.4,grain:0.12,vig:0.5});
  ok('filmnoir length', f.length===img.length);
  ok('filmnoir changed', !eq(f,img));
  // grayscale: r==g==b
  var gray=true; for(var i=0;i<f.length;i+=4){ if(Math.abs(f[i]-f[i+1])>1||Math.abs(f[i+1]-f[i+2])>1) gray=false; }
  ok('filmnoir grayscale', gray);
  console.log(T.join('\n'));
  console.log('FILMNOIR_TEST_DONE total='+T.length+' pass='+T.filter(function(x){return x[0]==='P';}).length);
})();
