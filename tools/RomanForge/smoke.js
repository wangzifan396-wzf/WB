...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('RomanForgePure exposed', typeof window.RomanForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('in').value='2024'; doc.getElementById('run').click();
  ok('output roman', (doc.getElementById('out').textContent||'')==='MMXXIV');
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('RomanForge smoke: '+pass+' passed, '+fail+' failed');
