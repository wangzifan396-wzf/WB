...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('HuffmanForgePure exposed', typeof window.HuffmanForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('compress output', /编码 bit 数/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('HuffmanForge smoke: '+pass+' passed, '+fail+' failed');
