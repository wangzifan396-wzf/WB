...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('MathForgePure exposed', typeof window.MathForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('expr').value='2 + 3 * 4^2';
  doc.getElementById('run').click();
  ok('output 50', (doc.getElementById('out').textContent||'')==='50');
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('MathForge smoke: '+pass+' passed, '+fail+' failed');
