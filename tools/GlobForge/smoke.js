...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('GlobForgePure exposed', typeof window.GlobForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('paths').value='a.js\nb.ts'; doc.getElementById('glob').value='*.js'; doc.getElementById('run').click();
  ok('output produced', doc.getElementById('out').textContent.indexOf('a.js')>=0);
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('GlobForge smoke: '+pass+' passed, '+fail+' failed');
