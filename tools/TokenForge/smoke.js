...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('TokenForgePure exposed', typeof window.TokenForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /估算 tokens:/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('TokenForge smoke: '+pass+' passed, '+fail+' failed');
