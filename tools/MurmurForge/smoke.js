...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('MurmurForgePure exposed', typeof window.MurmurForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('hash output', /0xc0363e43/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('MurmurForge smoke: '+pass+' passed, '+fail+' failed');
