...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('TomlForgePure exposed', typeof window.TomlForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('parse output', /users|server|title/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('TomlForge smoke: '+pass+' passed, '+fail+' failed');
