...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('ReadForgePure exposed', typeof window.ReadForgePure==='object');
  ok('calc btn', !!doc.getElementById('calc'));
  doc.getElementById('calc').click();
  ok('score shown', /Flesch/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('ReadForge smoke: '+pass+' passed, '+fail+' failed');
