...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('CsvForgePure exposed', typeof window.CsvForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('in').value='a,b\n1,2'; doc.getElementById('run').click();
  ok('output produced', (doc.getElementById('out').textContent||'')!=='结果会显示在这里' && doc.getElementById('out').textContent.length>0);
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('CsvForge smoke: '+pass+' passed, '+fail+' failed');
