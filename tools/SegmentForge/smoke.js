...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('SegmentForgePure exposed', typeof window.SegmentForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sum output', /sum \[1,5\] = 23/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('SegmentForge smoke: '+pass+' passed, '+fail+' failed');
