...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('HashRingForgePure exposed', typeof window.HashRingForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('ring output', /\u865a\u62df\u8282\u70b9|\u2192/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('HashRingForge smoke: '+pass+' passed, '+fail+' failed');
