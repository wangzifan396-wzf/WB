...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('MetaForgePure exposed', typeof window.MetaForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('output has og', (doc.getElementById('out').textContent||'').indexOf('og:title')>=0);
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('MetaForge smoke: '+pass+' passed, '+fail+' failed');
