...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('PasswordForgePure exposed', typeof window.PasswordForgePure==='object');
  ok('gen btn', !!doc.getElementById('gen'));
  doc.getElementById('gen').click();
  ok('out has entropy', /bits/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('PasswordForge smoke: '+pass+' passed, '+fail+' failed');
