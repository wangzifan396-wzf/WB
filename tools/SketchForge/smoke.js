...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('SketchForgePure exposed', typeof window.SketchForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /事件总数:/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('SketchForge smoke: '+pass+' passed, '+fail+' failed');
