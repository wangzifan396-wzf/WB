...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('FuzzyForgePure exposed', typeof window.FuzzyForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('match output', /Button\.tsx/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('FuzzyForge smoke: '+pass+' passed, '+fail+' failed');
