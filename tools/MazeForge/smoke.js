...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('MazeForgePure exposed', typeof window.MazeForgePure==='object');
  ok('solve btn', !!doc.getElementById('solve'));
  doc.getElementById('solve').click();
  ok('svg + path rendered', doc.getElementById('out').innerHTML.indexOf('<polyline')>-1);
  ok('no js errors', errs.length===0);
  console.log('MazeForge smoke: '+pass+' passed, '+fail+' failed');
