...
(function(){
  ...
  var ident=A.skewProcess(img,w,h,{angleX:0,angleY:0});
  ok('skew identity', eq(ident,img));
  var sk=A.skewProcess(img,w,h,{angleX:30});
  ok('skew length', sk.length===img.length);
  ok('skew changed', !eq(sk,img));
  console.log(T.join('\n'));
  console.log('SKEW_TEST_DONE total='+T.length+' pass='+T.filter(function(x){return x[0]==='P';}).length);
})();
