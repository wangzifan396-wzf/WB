...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('SoundexForgePure exposed', typeof window.SoundexForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('group output', /R163: Robert, Rupert/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('SoundexForge smoke: '+pass+' passed, '+fail+' failed');
