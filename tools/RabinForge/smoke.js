...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('RabinForgePure exposed', typeof window.RabinForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('match output', /"abra": 4 处/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('RabinForge smoke: '+pass+' passed, '+fail+' failed');
