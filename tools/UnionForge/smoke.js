...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('UnionForgePure exposed', typeof window.UnionForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /components/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('UnionForge smoke: '+pass+' passed, '+fail+' failed');
