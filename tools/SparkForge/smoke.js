...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('SparkForgePure exposed', typeof window.SparkForgePure==='object');
  ok('gen btn', !!doc.getElementById('gen'));
  doc.getElementById('gen').click();
  ok('svg rendered', doc.getElementById('out').innerHTML.indexOf('<svg')===0);
  ok('no js errors', errs.length===0);
  console.log('SparkForge smoke: '+pass+' passed, '+fail+' failed');
