...
(function(){
  ...
  var ident=A.mosaicProcess(img,w,h,{block:1});
  ok('mosaic block1 identity', eq(ident,img));
  var m2=A.mosaicProcess(img,w,h,{block:8});
  ok('mosaic length', m2.length===img.length);
  ok('mosaic changed', !eq(m2,img));
  console.log(T.join('\n'));
  console.log('MOSAIC_TEST_DONE total='+T.length+' pass='+T.filter(function(x){return x[0]==='P';}).length);
})();
