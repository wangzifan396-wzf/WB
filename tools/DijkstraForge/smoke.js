...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('DijkstraForgePure exposed', typeof window.DijkstraForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('path output', /距离|路径/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('DijkstraForge smoke: '+pass+' passed, '+fail+' failed');
