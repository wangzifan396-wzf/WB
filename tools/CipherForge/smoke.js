...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('CipherForgePure exposed', typeof window.CipherForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('in').value='Hello'; doc.getElementById('run').click();
  ok('output produced', doc.getElementById('out').textContent.length>0);
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('CipherForge smoke: '+pass+' passed, '+fail+' failed');
