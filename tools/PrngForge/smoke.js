...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('PrngForgePure exposed', typeof window.PrngForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sequence output', /#0\s+0\./.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('PrngForge smoke: '+pass+' passed, '+fail+' failed');
