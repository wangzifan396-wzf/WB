...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('XxhashForgePure exposed', typeof window.XxhashForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('hash output', /0x[0-9a-f]{8}/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('XxhashForge smoke: '+pass+' passed, '+fail+' failed');
