...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('BloomForgePure exposed', typeof window.BloomForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('membership output', /\u5b58\u5728|\u4e0d\u5b58\u5728/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('BloomForge smoke: '+pass+' passed, '+fail+' failed');
