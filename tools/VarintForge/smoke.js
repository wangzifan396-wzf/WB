...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('VarintForgePure exposed', typeof window.VarintForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('encode output', /300 -> ac 02/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('VarintForge smoke: '+pass+' passed, '+fail+' failed');
