...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('ArgForgePure exposed', typeof window.ArgForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /options:/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('ArgForge smoke: '+pass+' passed, '+fail+' failed');
