...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('PivotForgePure exposed', typeof window.PivotForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('pivot output', /TOTAL/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('PivotForge smoke: '+pass+' passed, '+fail+' failed');
