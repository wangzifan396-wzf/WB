...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('ChmodForgePure exposed', typeof window.ChmodForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('in').value='755'; doc.getElementById('run').click();
  ok('output has symbolic', (doc.getElementById('out').textContent||'').indexOf('rwxr-xr-x')>=0);
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('ChmodForge smoke: '+pass+' passed, '+fail+' failed');
