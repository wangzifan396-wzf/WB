...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('TopoForgePure exposed', typeof window.TopoForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('order output', /执行顺序/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('TopoForge smoke: '+pass+' passed, '+fail+' failed');
