...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('ExprForgePure exposed', typeof window.ExprForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('eval output', /= 121/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('ExprForge smoke: '+pass+' passed, '+fail+' failed');
