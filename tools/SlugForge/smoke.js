...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('SlugForgePure exposed', typeof window.SlugForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('in').value='Hello World'; doc.getElementById('run').click();
  ok('output produced', doc.getElementById('out').textContent.indexOf('hello-world')>=0);
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('SlugForge smoke: '+pass+' passed, '+fail+' failed');
