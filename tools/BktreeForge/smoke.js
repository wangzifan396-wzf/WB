...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('BktreeForgePure exposed', typeof window.BktreeForgePure==='object');
  ...
  ok('search output', /距离/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('BktreeForge smoke: '+pass+' passed, '+fail+' failed');
