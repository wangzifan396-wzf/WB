...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('LinearForgePure exposed', typeof window.LinearForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('fit output', /R² =/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('LinearForge smoke: '+pass+' passed, '+fail+' failed');
