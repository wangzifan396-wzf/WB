...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('HmacForgePure exposed', typeof window.HmacForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('hmac output', /HMAC-SHA256:\n[0-9a-f]{64}/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('HmacForge smoke: '+pass+' passed, '+fail+' failed');
