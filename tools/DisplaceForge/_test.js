...
(function(){
  ...
  var ident=A.displaceProcess(img,w,h,{amount:0});
  ok('displace amount0 identity', eq(ident,img));
  var d2=A.displaceProcess(img,w,h,{amount:40});
  ok('displace length', d2.length===img.length);
  ok('displace changed', !eq(d2,img));
  console.log(T.join('\n'));
  console.log('DISPLACE_TEST_DONE total='+T.length+' pass='+T.filter(function(x){return x[0]==='P';}).length);
})();
