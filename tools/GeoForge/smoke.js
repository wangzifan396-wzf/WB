...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('GeoForgePure exposed', typeof window.GeoForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('distance output', /大圆距离: 10[0-9]{2}(\.[0-9]+)? km/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('GeoForge smoke: '+pass+' passed, '+fail+' failed');
