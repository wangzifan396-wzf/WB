...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('DistForgePure exposed', typeof window.DistForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('lev output', /编辑距离: 3/.test(doc.getElementById('out').textContent));
  ok('jaro output', /Jaro 相似度/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('DistForge smoke: '+pass+' passed, '+fail+' failed');
