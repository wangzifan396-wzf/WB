...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('CombForgePure exposed', typeof window.CombForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('poker output', /C\(52,5\) = 2,598,960/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('CombForge smoke: '+pass+' passed, '+fail+' failed');
