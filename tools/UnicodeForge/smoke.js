...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('UnicodeForgePure exposed', typeof window.UnicodeForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('in').value='A'; doc.getElementById('mode').value='toEsc'; doc.getElementById('run').click();
  ok('output escape', (doc.getElementById('out').textContent||'').indexOf('\\u0041')>=0);
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('UnicodeForge smoke: '+pass+' passed, '+fail+' failed');
