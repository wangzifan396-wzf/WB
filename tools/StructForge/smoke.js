...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('StructForgePure exposed', typeof window.StructForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('in').value='{"a":1}'; doc.getElementById('run').click();
  ok('output produced', doc.getElementById('out').textContent.indexOf('interface')>=0);
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('StructForge smoke: '+pass+' passed, '+fail+' failed');
