...
(function(){
  ...
  ok('anaglyph length', A.anaglyphProcess(img,w,h,{sep:10}).length===img.length);
  ok('anaglyph changed', !eq(A.anaglyphProcess(img,w,h,{sep:10}), img));
  ok('anaglyph identity sep0', eq(A.anaglyphProcess(img,w,h,{sep:0}), img));
  console.log(T.join('\n'));
  console.log('ANAGLYPH_TEST_DONE total='+T.length+' pass='+T.filter(function(x){return x[0]==='P';}).length);
})();
