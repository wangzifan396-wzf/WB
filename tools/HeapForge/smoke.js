...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('HeapForgePure exposed', typeof window.HeapForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sort output', /\d/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('HeapForge smoke: '+pass+' passed, '+fail+' failed');
