...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('HllForgePure exposed', typeof window.HllForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /HLL 估算:/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('HllForge smoke: '+pass+' passed, '+fail+' failed');
