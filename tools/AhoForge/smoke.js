...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('AhoForgePure exposed', typeof window.AhoForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('match output', /命中/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('AhoForge smoke: '+pass+' passed, '+fail+' failed');
