...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('FaviconForgePure exposed', typeof window.FaviconForgePure==='object');
  ok('gen btn', !!doc.getElementById('gen'));
  doc.getElementById('in').value='A'; doc.getElementById('gen').click();
  ok('preview svg', !!doc.getElementById('preview').querySelector('svg'));
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('FaviconForge smoke: '+pass+' passed, '+fail+' failed');
