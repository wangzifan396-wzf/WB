...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('TarjanForgePure exposed', typeof window.TarjanForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('scc output', /强连通分量/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('TarjanForge smoke: '+pass+' passed, '+fail+' failed');
