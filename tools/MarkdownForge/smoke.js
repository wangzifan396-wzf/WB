...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('MarkdownForgePure exposed', typeof window.MarkdownForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('html output', /<h1>/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('MarkdownForge smoke: '+pass+' passed, '+fail+' failed');
