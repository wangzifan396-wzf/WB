...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('Base85ForgePure exposed', typeof window.Base85ForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('encode output', /^<~/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('Base85Forge smoke: '+pass+' passed, '+fail+' failed');
