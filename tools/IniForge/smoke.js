...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('IniForgePure exposed', typeof window.IniForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('parse output', /server|database/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('IniForge smoke: '+pass+' passed, '+fail+' failed');
