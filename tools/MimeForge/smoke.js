...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('MimeForgePure exposed', typeof window.MimeForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('in').value='json'; doc.getElementById('run').click();
  ok('output mime', (doc.getElementById('out').textContent||'')==='application/json');
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('MimeForge smoke: '+pass+' passed, '+fail+' failed');
