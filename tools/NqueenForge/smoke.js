...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('NqueenForgePure exposed', typeof window.NqueenForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('board rendered', doc.getElementById('out').innerHTML.indexOf('<svg')>-1 && /92 个解/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('NqueenForge smoke: '+pass+' passed, '+fail+' failed');
