...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('ULIDForgePure exposed', typeof window.ULIDForgePure==='object');
  ok('gen btn', !!doc.getElementById('gen'));
  doc.getElementById('gen').click();
  var out=doc.getElementById('out').textContent;
  ok('output produced', out.length>0 && out.indexOf('时间')>=0);
  ok('generated 26 chars', doc.getElementById('uid').value.length===26);
  ok('no js errors', errs.length===0);
  if(errs.length) console.error('  js errors:',errs);
  console.log('ULIDForge smoke: '+pass+' passed, '+fail+' failed');
