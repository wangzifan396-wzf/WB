...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('UnitForgePure exposed', typeof window.UnitForgePure==='object');
  ok('calc btn', !!doc.getElementById('calc'));
  doc.getElementById('calc').click();
  ok('converted', /=/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('UnitForge smoke: '+pass+' passed, '+fail+' failed');
