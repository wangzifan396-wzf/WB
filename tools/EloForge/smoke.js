...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('EloForgePure exposed', typeof window.EloForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('series output', /终局: A=/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('EloForge smoke: '+pass+' passed, '+fail+' failed');
