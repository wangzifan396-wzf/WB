...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('MinhashForgePure exposed', typeof window.MinhashForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('estimate output', /MinHash 估计/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('MinhashForge smoke: '+pass+' passed, '+fail+' failed');
