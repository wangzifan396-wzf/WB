...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('SimhashForgePure exposed', typeof window.SimhashForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('hash output', /SimHash A: [0-9a-f]{16}/.test(doc.getElementById('out').textContent));
  ok('hamming output', /海明距离/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('SimhashForge smoke: '+pass+' passed, '+fail+' failed');
