...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('HashForgePure exposed', typeof window.HashForgePure==='object');
  ok('calc btn', !!doc.getElementById('calc'));
  doc.getElementById('calc').click();
  ok('hash output 64hex', /^[0-9a-f]{64}$/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('HashForge smoke: '+pass+' passed, '+fail+' failed');
