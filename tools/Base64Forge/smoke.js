...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('Base64ForgePure exposed', typeof window.Base64ForgePure==='object');
  ok('enc btn', !!doc.getElementById('enc'));
  doc.getElementById('enc').click();
  ok('encode output', /^[A-Za-z0-9+\/=]+$/.test(doc.getElementById('out').textContent.trim()));
  ok('no js errors', errs.length===0);
  console.log('Base64Forge smoke: '+pass+' passed, '+fail+' failed');
