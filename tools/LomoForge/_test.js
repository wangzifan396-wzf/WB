...
(function(){
  ...
  ok('lomo length', A.lomoProcess(img,w,h,{sat:1.4,contrast:1.2,vig:0.55}).length===img.length);
  ok('lomo changed', !eq(A.lomoProcess(img,w,h,{sat:1.4,contrast:1.2,vig:0.55}), img));
  ok('lomo identity', eq(A.lomoProcess(img,w,h,{sat:1,contrast:1,vig:0}), img));
  console.log(T.join('\n'));
  console.log('LOMO_TEST_DONE total='+T.length+' pass='+T.filter(function(x){return x[0]==='P';}).length);
})();
