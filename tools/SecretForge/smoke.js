...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('SecretForgePure exposed', typeof window.SecretForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /疑似敏感串|未检测/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('SecretForge smoke: '+pass+' passed, '+fail+' failed');
