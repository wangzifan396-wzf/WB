...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('HtmlEntityForgePure exposed', typeof window.HtmlEntityForgePure==='object');
  ok('enc btn', !!doc.getElementById('enc'));
  doc.getElementById('enc').click();
  ok('encoded output', /\u0026/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('HtmlEntityForge smoke: '+pass+' passed, '+fail+' failed');
