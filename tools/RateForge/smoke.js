...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('RateForgePure exposed', typeof window.RateForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /ALLOW|DENY/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('RateForge smoke: '+pass+' passed, '+fail+' failed');
