...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('BackoffForgePure exposed', typeof window.BackoffForgePure==='object');
  ok('calc btn', !!doc.getElementById('calc'));
  doc.getElementById('calc').click();
  ok('output delay', /ms/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('BackoffForge smoke: '+pass+' passed, '+fail+' failed');
