...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('CrcForgePure exposed', typeof window.CrcForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('crc output', /十六进制/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('CrcForge smoke: '+pass+' passed, '+fail+' failed');
