...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('Sha3ForgePure exposed', typeof window.Sha3ForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('hash output', /[0-9a-f]{64}/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('Sha3Forge smoke: '+pass+' passed, '+fail+' failed');
