...
(function(){
  ...
  var none=A.liquifyProcess(img,w,h,{strokes:[],radius:10});
  ok('liquify no stroke identity', eq(none,img));
  var moved=A.liquifyProcess(img,w,h,{strokes:[{x:8,y:8,dx:3,dy:0,radius:5}],radius:5});
  ok('liquify length', moved.length===img.length);
  ok('liquify changed', !eq(moved,img));
  console.log(T.join('\n'));
  console.log('LIQUIFY_TEST_DONE total='+T.length+' pass='+T.filter(function(x){return x[0]==='P';}).length);
})();
