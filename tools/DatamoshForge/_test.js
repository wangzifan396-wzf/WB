...
(function(){
  ...
  ok('datamosh length', A.datamoshProcess(img,w,h,{band:16,ab:8,smear:0.5}).length===img.length);
  ok('datamosh changed', !eq(A.datamoshProcess(img,w,h,{band:16,ab:8,smear:0.5}), img));
  ok('datamosh identity', eq(A.datamoshProcess(img,w,h,{band:16,ab:0,smear:0}), img));
  console.log(T.join('\n'));
  console.log('DATAMOSH_TEST_DONE total='+T.length+' pass='+T.filter(function(x){return x[0]==='P';}).length);
})();
