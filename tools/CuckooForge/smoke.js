...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('CuckooForgePure exposed', typeof window.CuckooForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('query output', /可能存在/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('CuckooForge smoke: '+pass+' passed, '+fail+' failed');
