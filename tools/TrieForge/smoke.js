...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('TrieForgePure exposed', typeof window.TrieForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /startsWith/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('TrieForge smoke: '+pass+' passed, '+fail+' failed');
