...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('UtmForgePure exposed', typeof window.UtmForgePure==='object');
  ok('build btn', !!doc.getElementById('build'));
  doc.getElementById('build').click();
  ok('built url', /utm_source=newsletter/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('UtmForge smoke: '+pass+' passed, '+fail+' failed');
