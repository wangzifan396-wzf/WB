...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('BreakerForgePure exposed', typeof window.BreakerForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /->/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('BreakerForge smoke: '+pass+' passed, '+fail+' failed');
