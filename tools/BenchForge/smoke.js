...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('BenchForgePure exposed', typeof window.BenchForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('stats output', /mean/.test(doc.getElementById('out').textContent));
  ok('outlier detected', /30.5/.test(doc.getElementById('out').textContent));
  doc.getElementById('cmp').click();
  ok('compare output', /速度比/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('BenchForge smoke: '+pass+' passed, '+fail+' failed');
