...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('FigletForgePure exposed', typeof window.FigletForgePure==='object');
  ok('gen btn', !!doc.getElementById('gen'));
  doc.getElementById('gen').click();
  ok('art rendered', doc.getElementById('out').textContent.indexOf('#')>=0);
  ok('no js errors', errs.length===0);
  console.log('FigletForge smoke: '+pass+' passed, '+fail+' failed');
