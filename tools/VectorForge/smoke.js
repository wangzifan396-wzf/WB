...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('VectorForgePure exposed', typeof window.VectorForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('sim output', /cosine = /.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('VectorForge smoke: '+pass+' passed, '+fail+' failed');
