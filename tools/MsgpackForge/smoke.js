...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('MsgpackForgePure exposed', typeof window.MsgpackForgePure==='object');
  ok('enc btn', !!doc.getElementById('enc'));
  doc.getElementById('enc').click();
  ok('encode output', /字节/.test(doc.getElementById('out').textContent));
  doc.getElementById('dec').click();
  ok('decode output', /nano/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('MsgpackForge smoke: '+pass+' passed, '+fail+' failed');
