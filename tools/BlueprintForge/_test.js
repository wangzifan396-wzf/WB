...
(function(){
  ...
  var b1=A.blueprintProcess(img,w,h,{edge:30});
  ok('blueprint length', b1.length===img.length);
  ok('blueprint changed', !eq(b1,img));
  var blue=true;
  for(var i=0;i<b1.length;i+=4){ if(!(b1[i+2]>=b1[i+1] && b1[i+1]>=b1[i])) blue=false; }
  ok('blueprint bluish', blue);
  console.log(T.join('\n'));
  console.log('BLUEPRINT_TEST_DONE total='+T.length+' pass='+T.filter(function(x){return x[0]==='P';}).length);
})();
