...
setTimeout(()=>{ const doc=window.document; let pass=0,fail=0;
  ...
  ok('MerkleForgePure exposed', typeof window.MerkleForgePure==='object');
  ok('run btn', !!doc.getElementById('run'));
  doc.getElementById('run').click();
  ok('root + verify output', /Merkle Root:/.test(doc.getElementById('out').textContent) && /PASS/.test(doc.getElementById('out').textContent));
  ok('no js errors', errs.length===0);
  console.log('MerkleForge smoke: '+pass+' passed, '+fail+' failed');
